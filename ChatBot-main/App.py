...
def removeCommonWords(message: str) -> str:
    commonWords: set = {"an", "a", "i", "he", "she", "they", "it", "them", "his", "her", "them", "its", 'hers', 'and',
                        ...
                        "he's", "she's", "you", "your", "yours", "you've","me"}
...
def getMatches(text: str, possibilities: list, n: float = float("inf"), cutoff: float = 0.6):
    ...
    for possiblity in possibilities:
        possiblityF = format_question(possiblity)
        possibleNgram = set(ngram3.split(possiblityF))
        possibleNgram4 = set(ngram4.split(possiblityF))
...
def membership(message: str) -> float:
    ...
    matches: list = getMatches(message, questions.keys(), cutoff=0.5)
    if len(matches) == 0:
        return -1
...
if __name__ == "__main__":
    ...
    if MODE.lower() == "train":
        trainMode = input("Train :: [Train] , [Self Train] : ")
        if trainMode.lower() == "self train":
            selfTrain()
        else:
            Train()
    elif MODE == "fun":
        while True:
            ...
            text = getBestAnswer(input("D.N.C AI: "))
            print(text)
            engine = pyttsx3.init()
